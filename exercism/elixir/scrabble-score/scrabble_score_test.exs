...
defmodule ScrabbleScoreTest do
  ...
  test "missing tests" do
    # letters that were not covered by the original suite
    assert Scrabble.score("g") == 2
    assert Scrabble.score("d") == 2
    assert Scrabble.score("m") == 3
    assert Scrabble.score("v") == 4
    assert Scrabble.score("w") == 4
    assert Scrabble.score("j") == 8
  end
end
